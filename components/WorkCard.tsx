    ...
    <>
      <Link href={`/works/${work.url}`}>
        <div className="w-72 h-full rounded-b overflow-hidden shadow-lg bg-slate-100 hover:bg-slate-200">
          <Image className="w-full" src={work.pic} alt={work.name} />
          ...
          <div className="px-6 pt-4 pb-2">
            {work.categories.map((category) => (
              <span key={category} className="inline-block bg-amber-500  px-3 py-1 text-sm font-semibold text-gray-700 mr-2 mb-2">
                {category}
              </span>
            ))}
